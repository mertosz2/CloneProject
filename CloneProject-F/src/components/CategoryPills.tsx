...
export function CategoryPills({ categories, selectedCategory, onSelect }: CategoryPillsProps) {
    ...
    useEffect(() => {
        ...
        const observer = new ResizeObserver(entries => {
            const contianer = entries[0]?.target
            if(contianer == null) return

            setLeftVisible(translate > 0)
            setRightVisible(
                translate + contianer.clientWidth < contianer.scrollWidth
    ...
    return <div 
        ...
        <div
            ...
            {categories.map(categories => (
                <Button key={categories} variant={selectedCategory === categories ? "dark" : "default"} onClick={() => onSelect(categories)}>{categories}</Button>
        ...
        {isRightVisible && (
            <div
                className="absolute right-0 top-0 bg-gradient-to-l from-white from-50% to -transparent w-24 h-full flex justify-end">
                <Button
                    variant="ghost"
                    size="icon"
                    className="h-full aspect-square w-auto p-1.5"
